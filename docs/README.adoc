...
We have automation in place for provisioning a database server and adding a database to it. Now if only there were a way
to bring that automation into OpenShift for self-service consumption by the application teams... (hint: that's exactly
what we're going to do).

=== 3.1 Automation Service Broker

The https://docs.openshift.com/container-platform/3.10/architecture/service_catalog/ansible_service_broker.html[OpenShift Ansible Broker]
(or http://automationbroker.io/[Automation Broker]) provides a way to deploy playbooks via the OpenShift service catalog.
Playbooks are packaged as https://docs.openshift.com/container-platform/3.10/apb_devel/[Ansible Playbook Bundles] which
are lightweight images containing parametrized playbooks. Let's try our hand at converting our
...
note are the `apb.yml` which specifies required parameters, `Dockerfile` which specifies how to build the image, and the
`playbooks` folder which is how OpenShift will invoke certain "actions" on the APB (these can probably stay as-is).
...
the broker (`oc logs --since=30s dc/asb -n openshift-ansible-service-broker`). The bootstrap command is also run on a
timer configured in the brokers config which can be viewed/edited in the web-console of the broker namespace or by
running `oc edit cm/broker-config -n openshift-ansible-service-broker` (it's much easier to do in the web console due to
formatting).
