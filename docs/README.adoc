...
spring:
  ...
  datasource:
    url: jdbc:mysql://ec2-18-234-37-92.compute-1.amazonaws.com:3306/widgettest <- this is the line
```
...
Now we should see a lot (relatively) more stuff in our database! So we can all stop and go home right? Wrong there is a
...
```
$ oc login https://ec2-18-234-37-92.compute-1.amazonaws.com -u admin -p redhat01 <- Ensure to use correct OpenShift cluster
...
```
$ oc secrets new generic mysql --from-generic=database-user=widget --from-generic=database-password=widget01 \
    --from-generic=database-name=widget
```
https://docs.openshift.com/container-platform/3.10/dev_guide/integrating_external_services.html[exposing external services]
